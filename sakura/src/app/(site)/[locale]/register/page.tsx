...
export default async function RegisterPage({
  ...
  return (
    ...
    >
      ...
      {/* 海外の購入者に住所・電話番号・フリガナは求めない。メールとパスワードだけ */}
      <form action={signUpAction} className="flex flex-col gap-4">
        ...
        <Field
          ...
          required
        />
        <Field id="password" label={a('password')} type="password" autoComplete="new-password" required />
        <p className="-mt-2 text-[11px] text-ink-muted">{a('passwordHint')}</p>
        <button
          type="submit"
          disabled={!ready}
