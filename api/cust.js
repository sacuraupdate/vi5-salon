...
module.exports=async(req,res)=>{
  ...
  try{
    ...
    if(op==='register'){const cust=b.customer||{};const P=norm(cust.phone);if(!P||!cust.name){res.status(400).json({ok:false,error:'bad'});return;}
      ...
      d.nextCustNo=d.nextCustNo||(Object.keys(d.customers).length+1);cust.no=d.nextCustNo++;cust.phone=P;cust.joined=cust.joined||today();cust.notices=cust.notices||[];
      d.customers[P]=cust;
      const rp=norm(b.referrerPhone);if(rp&&d.customers[rp]&&rp!==P){d.referrals=d.referrals||[];d.referrals.push({id:'rf'+Date.now(),newPhone:P,newName:cust.name,refPhone:rp,refName:d.customers[rp].name||'',at:Date.now(),status:'pending'});}
    ...
    if(op==='referral'){const P=norm(b.phone),rp=norm(b.referrerPhone);if(!P||!rp||!d.customers||!d.customers[P]||!d.customers[rp]||rp===P){res.status(200).json({ok:false});return;}
      d.referrals=d.referrals||[];if(!d.referrals.some(r=>r.newPhone===P)){d.referrals.push({id:'rf'+Date.now(),newPhone:P,newName:d.customers[P].name||'',refPhone:rp,refName:d.customers[rp].name||'',at:Date.now(),status:'pending'});await kvSet('salon:data',d);}res.status(200).json({ok:true});return;}
    if(op==='readNotices'){const P=norm(b.phone);const c=d.customers&&d.customers[P];if(!c||String(c.password==null?'':c.password)!==String(b.pw||'')){res.status(401).json({ok:false});return;}
      (c.notices||[]).forEach(n=>n.read=true);await kvSet('salon:data',d);res.status(200).json({ok:true});return;}
